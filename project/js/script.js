...
// Load recipes from default + database + localStorage
async function loadRecipes() {
    ...
    let dbRecipes = [];
    try {
        const response = await fetch('includes/get_recipes.php');
        const data = await response.json();
        if (Array.isArray(data)) {
            dbRecipes = data;
        }
    } catch (e) {
        console.error("Failed to fetch recipes from DB:", e);
    }

    // Combine all sources: Defaults (filtered), Database Recipes, and Local Storage (user added via old system)
    recipes = [...filteredDefaults, ...dbRecipes, ...userRecipes];
    
    // Refresh UI if we are on a page with a recipe grid
    const recipeGrid = document.getElementById('recipeGrid');
    if (recipeGrid) {
        // Check if this grid has a data-limit attribute (home page)
        const limit = recipeGrid.dataset.limit ? parseInt(recipeGrid.dataset.limit) : null;
        displayRecipes(recipes, limit);
    }
}

// --- Initialization ---
document.addEventListener('DOMContentLoaded', () => {
    loadRecipes(); // Initialize data and UI
...
function displayRecipes(recipeList, limit = null) {
    ...
    // Get limit from data attribute if it exists (for home page)
    const gridLimit = grid.dataset.limit ? parseInt(grid.dataset.limit) : null;
    const finalLimit = (gridLimit !== null && !isNaN(gridLimit)) ? gridLimit : limit;
    
    // Limit recipes if specified (for featured section on home page)
    const displayList = (finalLimit && finalLimit > 0) ? recipeList.slice(0, finalLimit) : recipeList;
...
window.deleteRecipe = (id) => {
    if (confirm("Are you sure you want to delete this recipe?")) {
        // Try removing from userRecipes (localStorage)
        ...
        if (userRecipes.length < originalUserCount) {
            ...
            if (defaultRecipes.some(r => r.id === id)) {
                ...
                // If it's a DB recipe, we'd need a delete_recipe.php (not implemented yet)
                alert("Database recipes cannot be deleted from the frontend yet.");
                return;
            }
        }
    ...
    forms.forEach(form => {
        form.addEventListener('submit', (e) => {
            if (!form.checkValidity()) {
                ...
                let endpoint = '';
                if (pageHref.includes('login.php')) endpoint = 'auth/login.php';
                else if (pageHref.includes('register.php')) endpoint = 'auth/register.php';
                else if (pageHref.includes('contact.php')) endpoint = 'includes/process_contact.php';
                else if (pageHref.includes('submit_recipe.php')) endpoint = 'includes/submit_recipe_handler.php';
                ...
                if (endpoint) {
                    ...
                    fetch(endpoint, { method: 'POST', body: formData })
                        ...
                        .then(data => {
                            if (data.success) {
                                let redirect = null;
                                if (endpoint === 'auth/login.php') redirect = 'dashboard.php';
                                else if (endpoint === 'auth/register.php') redirect = 'login.php';
                                else if (endpoint === 'includes/submit_recipe_handler.php') redirect = 'recipes.php';
                                
                                showMessageAndRedirect(data.message, redirect);
                                // Reload recipes if we just submitted one
                                if (endpoint === 'includes/submit_recipe_handler.php') loadRecipes();
                            } else {
                                showMessageAndRedirect(data.message, null, true);
                            }
                        })
                        .catch(err => {
                            console.error(err);
                            showMessageAndRedirect('Network error. Database might not be running.', null, true);
                        });
                } else {
                    const action = form.getAttribute('action');
                    showMessageAndRedirect('Form submitted successfully!', (action && action !== '#') ? action : null);
                }
            }
        });
    });
